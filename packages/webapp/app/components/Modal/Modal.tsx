...
<div className="fixed inset-0 flex w-screen items-center justify-center p-4">
  <Dialog.Panel className="mx-auto max-w-xl rounded bg-white p-4 ">
    ...
    <Dialog.Description className="text-justify text-gray-500 mt-4">
      ...
      <p className='text-sm'>
        {t('modal.betaDescription')}
      </p>
      <h3 className='mt-2 text-sm font-bold'>
        {t('modal.currentBetaFeatures')}:
      </h3>
      <ul className="ml-2">
        <li className="text-sm">
          <span className="font-bold">Network view</span> - visualize relationships between people based on their publications.
        </li>
      </ul>
    </Dialog.Description>
  </Dialog.Panel>
 ...
 */
export function Modal({ isOpen, setIsOpen, type } : { isOpen: boolean, setIsOpen: (isOpen: boolean) => void, type: keyof typeof modalTypes }) {
  return (
    <Dialog
      open={isOpen}
